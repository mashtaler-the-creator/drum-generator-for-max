...
**Kick.** Short (60–120 ms body), pitch-enveloped from a higher pitch to the fundamental in the first 10–20 ms, hard-clipped and saturated; every producer designs their own so kicks do not phase in DJ mixes (Landry). Start from a **Roland TR-909** (owned) or a LinnDrum kick (Attack's Thumping Techno uses a LinnDrum kick through compression and analogue overdrive — owned) and layer a sine sub; a **Jomox Xbase09 / Mbase01** (owned; I Hate Models used a Jomox Xbase 999) gives the analogue punch and pitch sweep natively. The rumble is not a sample — build it as a parallel return from the kick. Sonic Academy Kick 2 is the standard synthesised alternative (get elsewhere). Keep everything under 150 Hz mono.

**Snare / clap / rim.** Noise-synth or **909** snare on 2 and 4 for the industrial lane (Attack: "synth noise generator, analogue overdrive"); **TR-808** snare pitched down nearly an octave and low-passed for the thumping lane (owned); **909 rim** tuned high and low with heavy overdrive as bar-start accents (owned). Claps are 909/808 stacked with a snare and a room reverb, or a big rave clap (Novation Drumstation / Quasimidi 309 — owned — give period-correct rave claps).

**Hats.** 909 closed hat, often resampled through an **E-mu SP-1200** for 12-bit grit (Attack's Industrial Techno runs the 909 hat through the SP-1200 — both owned), low-passed with resonance and ~8 dB drive; 909 open hat compressed; **909 ride** high-passed above 1 kHz and sidechained to the kick. Machinedrum (owned) metallic hats fit the schranz lane.

**Percussion.** Bitcrushed low toms (909/808 through Decimort-style 6-bit crushing), woodblock/clave, congas from **TR-727 / TR-505** (owned) filtered and compressed into loops, metallic clanks and found sound in the cowbell slot, shakers high-passed at 24 dB/oct. **Elektron Machinedrum** and **Korg ER-1** (owned) are ideal for synthesised metallic perc; **Simmons SDS** toms (owned) for the descending tom cascades.
