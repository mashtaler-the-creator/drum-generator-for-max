...
**Kick.** An 808-style kick with a powerful, low-frequency punch drives every lane; automotivo and beat-de-faixa voicings push it harder and add kick rolls, beat bolha keeps it soft and downbeat-only. Program from a **Roland TR-808** sub layer.

**Snare/clap/rim.** The clap (`perc` slot) is the real backbeat almost everywhere, landing on 2 and 4 with syncopated ghost claps between; mandelão instead favours a clipped **rim** hit — "clipped toms and rims" is literally how the São Paulo microstyle's percussion signature is described. A **TR-808** supplies both.
